...
export const renderAdminPage = (options: { nonce?: string } = {}) => {
  const body = `
    <div class="grid two">
      <section class="card stack">
        <h2>Admin Access</h2>
        <p class="subtle">Enter the admin token configured on the backend to review submitted registrar applications.</p>
        <label>Admin Token<input id="adminToken" type="password" placeholder="ADMIN_API_TOKEN" /></label>
        <div class="row">
          <label>
            Status Filter
  ...
  const scripts = `
    const state = { token: "", selectedApplicationId: "" };
